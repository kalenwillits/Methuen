...
then: [ActionPoints] -= 1; [HarvestCount] += 1
else: (None)
```
(The `then` clause shows effects that always execute: reducing AP and incrementing counter)
...
Always Execute (Effect)
[ActionPoints] -= 1; [HarvestCount] += 1
...
then: #Always Execute
```
(Components can be reused across actions. The `then` component executes regardless of condition outcome)
