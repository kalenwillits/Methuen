...
- Resource changes (`[ActionPoints] -= 1`, `[Gold] += 5`, `[Counter] += 1`)
- Status effects (`Apply burning`, `Remove marker from tile`)
- Chained actions (`Execute follow-up action`)
...
**Note**: The (then) component always executes regardless of whether the condition was true or false. In this action, resource expenditure is placed in (then), but this is one pattern among many.
...
- Resource changes (expenditure, gain, transfers)
- Status effects or state changes
- Chained actions or follow-up operations
...
#### Resource Transfer (Expenditure in Do)
...
Execution: If you have enough gold, transfer it. Resource expenditure is in (do), not (then). This means you only spend gold if you have enough.
...
#### Movement with Energy Expenditure
...
Execution: If you have movement, move. Energy reduces by 1 regardless of condition outcome.
...
Execution: Success grants access. Failure increases security. ActionPoints always reduce by 1.
