...
- Always: Reduce ActionPoints by 2
...
- Multiple actions can reference shared effect components
...
- Always: Reduce ActionPoints by 3
...
- Resource expenditure in (do) - only consumed on success
...
- Should be balanced with action's overall resource impact
- Can include multiple effects

### Always Effect (then)

- Effects that execute regardless of condition outcome
- Common uses: resource changes, status effects, chained actions, counters
- Creates certainty in action outcomes
- Can be resource expenditure, resource gain, or state changes
...
### Where should I put resource expenditure?

**Note**: Resource expenditure is ONE common pattern among many for action design. Methuen is a simulation framework - these are design options, not prescriptive rules. You can also use `then` for resource gain, status effects, chained actions, counters, or any other effects.

**Resource Expenditure in (then) - Always Occurs:**
- Use when: Attempting the action itself has consequences
- Creates: Strategic positioning, resource tension
- Example: Spell reduces mana even if target is out of range

**Resource Expenditure in (do) - Only on Success:**
